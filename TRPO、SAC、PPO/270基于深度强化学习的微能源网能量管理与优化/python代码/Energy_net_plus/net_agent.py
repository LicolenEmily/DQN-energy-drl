...
class NetAgent(object):
    def __init__(
        ...
        checkpoint = tf.train.get_checkpoint_state("net")
    ...
    def _build_net(self):
        def build_layers(s, c_name, n_l1, w_initializer, b_initializer):
            ...
            with tf.variable_scope('l2'):
                w2 = tf.get_variable(name='w1', shape=[n_l1, n_l1], initializer=w_initializer,
                                     collections=c_name)
                b2 = tf.get_variable(name='b1', shape=[1, n_l1], initializer=b_initializer, collections=c_name)
                l2 = tf.nn.relu(tf.matmul(l1, w2) + b2)
            with tf.variable_scope('l3'):
                w3 = tf.get_variable(name='w2', shape=[n_l1, self.n_actions], initializer=w_initializer,
                                     collections=c_name)
                b3 = tf.get_variable(name='b2', shape=[1, self.n_actions], initializer=b_initializer,
                                     collections=c_name)
    ...
    def save_model(self):
        self.saver.save(self.sess, 'net/my_net.ckpt')
    ...
    def learn(self):
        ...
        _, self.cost = self.sess.run([self.train_op, self.loss], feed_dict={self.s: batch_memory[:, :self.n_features],
